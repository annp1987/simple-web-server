...
from web_server.db import context
from web_server.db import exceptions
from web_server.db.sqlalchemy import api as db_api, migration
from web_server import version

CONF = cfg.CONF

dbapi = db_api.get_instance()

# NOTE: This is a list of functions to perform online data migrations
...
class DBCommand(object):
    ...
    def check_obj_versions(self, ignore_missing_tables=False):
        ...
        of ovs-firewall. It does this by comparing the objects' .version field
        ...
        try:
            if not dbapi.check_versions(ignore_models=ignore_models):
                msg = ('The database is not compatible with this '
                       'release of ironic (%s). Please run '
                       '"ovs-firewall-db online_data_migrations" using '
                       'the previous release.\n'
                       % version.version_info.release_string())
        except exceptions.DatabaseVersionTooOld:
            msg = ('The database version is not compatible with this '
                   'release of ovs-firewall (%s). This can happen if you are '
                   ...
                   'Please run "ovs-firewall-db upgrade" using the previous '
    ...
    def _run_migration_functions(self, context, max_count, options):
        ...
        for migration_func_obj, migration_func_name in ONLINE_MIGRATIONS:
            ...
            try:
                ...
                print("Error while running %(migration)s: %(err)s."
                      % {'migration': migration_func.__name__, 'err': e},
                      file=sys.stderr)
                raise

            print('%(migration)s() migrated %(done)i of %(total)i objects.'
    ...
    def _run_online_data_migrations(self, max_count=None, options=None):
        ...
        if max_count is None:
            max_count = 50
            print('Running batches of %i until migrations have been '
                  'completed.' % max_count)
...
def add_command_parsers(subparsers):
    ...
    parser = subparsers.add_parser(
        ...
        help="Upgrade the database schema to the latest version. "
             ...
             "'ovs-firewall-db online_data_migrations' command should be run "
             "using the previous version of ovs-firewall, before upgrading and "
    ...
    parser = subparsers.add_parser(
        ...
        help="Perform online data migrations for the release. If "
             ...
             "using the previous release of ovs-firewall, before upgrading and "
             "running this command.")

    parser.add_argument(
        '--max-count', metavar='<number>', dest='max_count', type=int,
        help="Maximum number of objects to migrate. If unspecified, all "
             "objects are migrated.")

    parser.add_argument(
        '--option', metavar='<migration.opt=val>', action='append',
        dest='options', default=[],
        help="Options to pass to the migrations in the form of "
...
def prepare_service(argv=None):
    ...
    log.setup(CONF, 'ovs-firewall')
    CONF(argv[1:], project='ovs-firewall',
